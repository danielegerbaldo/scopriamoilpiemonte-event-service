...
public interface EventoRepository extends JpaRepository<Evento, Long> {
    ...
    public final static String queryFindEventiUtenteNonIscritto = "SELECT evento.* FROM evento where evento.id NOT IN (SELECT evento_iscritti.evento_id FROM evento_iscritti where evento_iscritti.iscritti = 2)";
    ...
    @Query(value = "SELECT evento.* FROM evento where evento.id NOT IN (SELECT evento_iscritti.evento_id FROM evento_iscritti where evento_iscritti.iscritti = ?1)", nativeQuery = true)
    ...
    @Query(value = "select evento.* from evento where evento.id in (select evento_iscritti.evento_id from evento_iscritti where evento_iscritti.iscritti = ?1)", nativeQuery = true)
